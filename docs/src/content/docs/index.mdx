...
template: splash
# The page title matches the site title, so Starlight's default composition
# renders "CodeGen-Sandbox | CodeGen-Sandbox" in the tab and in search results. Override the
# whole title for this page only.
#
# `next: false` removes Starlight's pagination footer. It is a docs affordance
# — "Next: Getting Started" — and a landing page is not part of that sequence;
# no other product site's home page has one.
next: false
head:
  - tag: title
    content: CodeGen-Sandbox — safe codegen tools for PromptKit agents
hero:
...
<CardGrid>
  ...
  <Card title="Vendor MCP for the web">
